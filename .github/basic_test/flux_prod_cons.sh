...
server_cmd="flux submit --flags=waitable -N 1 -n $NP_SERVER --output=server.out --error=server.err $SPACK_VIEW/bin/dspaces_server $HG_CONNECTION_STR"
echo ${server_cmd}
...
for (( i=1; i<=$NWRITERS; i++ )); do
  ...
  for (( j=1; j<=$NREADERS_PER_WRITER; j++ )); do
    ...
    consumer_cmd="flux submit --flags=waitable -N 1 -n $NP_READER --output=consumer${i}_${j}.out --error=consumer${i}_${j}.err ./consumer dataspaces $client_id $group_id ./compute.py analyze $NSTEPS"
    echo ${consumer_cmd}
