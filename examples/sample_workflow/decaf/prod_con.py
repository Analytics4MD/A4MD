...
for prod_id in range(nproducers):
	process_id += 1
	prod = wf.Node("producer"+str(prod_id), start_proc=process_id, nprocs=1, func='', cmdline='./producer decaf prod_con.json ./load.py extract_frame 5 200 0')
	outPort = prod.addOutputPort("out")

	for con_id in range(nconsumers_per_producer):
		process_id += 1
		con = wf.Node("consumer"+str(prod_id)+"."+str(con_id), start_proc=process_id, nprocs=1, func='', cmdline='./consumer decaf prod_con.json ./compute.py analyze 5')
