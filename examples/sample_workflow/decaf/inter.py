...
for prod_id in range(nproducers):
	process_id += 1
	prod = wf.Node("producer"+str(prod_id), start_proc=process_id, nprocs=1, func='', cmdline='./producer decaf inter.json ./load.py extract_frame 5 200 0')
	prod.addOutputPort("out")

	for inter_id in range(nintermediators_per_producer):
		process_id += 1
		inter = wf.Node("intermediator"+str(prod_id)+"."+str(inter_id), start_proc=process_id, nprocs=1, func='', cmdline='./intermediator decaf inter.json ./forward.py direct 5')
		...
		for con_id in range(nconsumers_per_intermediators):
			process_id += 1
			con = wf.Node("consumer"+str(prod_id)+"."+str(inter_id)+"."+str(con_id), start_proc=process_id, nprocs=1, func='', cmdline='./consumer decaf inter.json ./compute.py analyze 5')
			con.addInputPort("in")
